    ...
    return (
        <div className={s.pageContainer}>
            <ul className={s.pagesList} >
                {(currentPage > 1)&&<li className={ s.itemList} onClick={currentPage > 1 ? ()=>handleClick(currentPage-1):null}>&lt;</li>}
                ...
                { (currentPage < (pageNumbers.length)) && <li className={ s.itemList} onClick={currentPage < (pageNumbers.length) ? ()=>handleClick(currentPage+1):null}>&gt;</li>} 
            </ul>
        </div>
    )
}
